...
/* ❌ REMOVED transporter.verify() (CAUSES RENDER ISSUES) */
...
async function sendOrderEmail(to, order) {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<ul>${itemsList}</ul><p>Total: ₵${order.total}</p>`
    });
...
async function sendStatusEmail(to, order) {
  try {
    ...
    const color = colorMap[order.status] || "#888";

    await transporter.sendMail({
      from: `ChopSpot <${EMAIL_USER}>`,
      to,
      subject: `Order Update — ${order.status.toUpperCase()}`,
      html: `<h2 style="color:${color}">${order.status.toUpperCase()}</h2>`
    });

    console.log("✅ Status email sent:", to);
  } catch (err) {
